...
export async function clientLoader({ request }: Route.ClientLoaderArgs) {
  try {
    ...
    if (staffError || staff?.role !== "CSO") {
      console.log("Unauthorized access attempt to CSO dashboard");
    ...
    // Fetch passes approved by DSA today (regardless of current status)
    ...
    const { data: dsaDeniedToday, error: dsaDeniedError } = await supabase
      .from("pass")
      .select("id")
      .eq("status", "rejected")
    ...
    if (dsaDeniedError) {
      ...
      console.log("Error fetching denied passes:", dsaDeniedError);
      return;
    }
    ...
    return {
      stats: {
        ...
        dsaDeniedToday: dsaDeniedToday?.length || 0,
    ...
    toast.error("Error Loading CSO dashboard data");
    console.error("Error loading CSO dashboard:", error);
    return {
      stats: {
        pendingApproval: 0,
        dsaApprovedToday: 0,
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-muted rounded-lg">
                  ...
                  <div>
                    <p className="font-medium">
                      {selectedPass.status === "dsa_approved" ? (
                        <Badge
                          variant="outline"
